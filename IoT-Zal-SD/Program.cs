...
using Opc.UaFx.Client;
    ...
    private static async Task Main(string[] args)
    {
        try
        {
            ...
            using (var opcClient = new OpcClient(Resource.opcClientServer))
            {
                ...
                RegistryManager registryManager = RegistryManager.CreateFromConnectionString(Resource.ownerConnectionString);
                IEnumerable<Device> devices = await registryManager.GetDevicesAsync(int.Parse(Resource.iotDevicesMaxCount));
                ...
                for (int i = 0; i < deviceIds.Count; i++)
                {
                    ...
                    {
                        DeviceClient deviceClient = DeviceClient.CreateFromConnectionString(Resource.deviceConnectionString, deviceIds[i]);
